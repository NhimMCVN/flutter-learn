import 'package:first_flutter_app/ui/todo/ui/category_item.dart';
import 'package:flutter/material.dart';

class ListCategory extends StatefulWidget {
  final EnumInputMoney type;
  final void Function(Category cate)? onSelectedCate;
  final String? initSelectedKey;

  ListCategory(
    this.type, {
    this.onSelectedCate,
    this.initSelectedKey,
    super.key,
  });

  @override
  State<ListCategory> createState() {
    return _ListCategoryState();
  }
}

class _ListCategoryState extends State<ListCategory> {
  List<Category> listCategory = [];
  Category? selectedCate;

  @override
  void initState() {
    super.initState();
    listCategory = widget.type == EnumInputMoney.Spending
        ? listSpendingCate
        : listRevenueCate;
    final Category? foundCate = listCategory.firstWhere(
      (ele) => ele.name == widget.initSelectedKey,
      orElse: () => Category('', Icons.help_outline),
    );
    selectedCate = foundCate;
  }

  @override
  Widget build(BuildContext context) {
    return GridView.builder(
      padding: const EdgeInsets.all(8),
      shrinkWrap: true,
      physics: NeverScrollableScrollPhysics(),
      gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
        crossAxisCount: 4,
        crossAxisSpacing: 8,
        mainAxisSpacing: 8,
        childAspectRatio: 1,
      ),
      itemCount: listCategory.length,
      itemBuilder: (context, index) {
        final item = listCategory[index];
        return CategoryItem(
          item,
          onSelected: (cate) {
            widget.onSelectedCate?.call(cate);
            setState(() {
              selectedCate = cate;
            });
          },
          initValue: item.name == (selectedCate?.name ?? ""),
        );
      },
    );
  }
}
